...
public interface PromocionDao extends JpaRepository<Promocion, Integer> {
	...
	@Query(value="SELECT COUNT(*) FROM promociones p WHERE p.id_restaurante = :idRestaurante AND p.id_producto = :idProducto", nativeQuery = true)
	public int comprobarExistenciaPromocion(@Param("idRestaurante") int idRestaurante, @Param("idProducto") int idProducto);
	
	@Query(value="SELECT * FROM promociones p WHERE p.id_restaurante = :idRestaurante AND p.id_producto = :idProducto", nativeQuery = true)
	public Promocion buscarPromocion(@Param("idRestaurante") int idRestaurante, @Param("idProducto") int idProducto);
	
	@Query(value="SELECT * FROM promociones p WHERE p.id_restaurante = :idRestaurante", nativeQuery = true)
